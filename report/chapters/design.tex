\chapter{Designing the System}
	...
	\section{Storage Encryption}
		...
		\subsection{Key Management}
			...
			\subsubsection{Choosing an Encryption Algorithm}
				...
				On the other hand the National Institute of Standards and Technology \emph{(NIST)} -- strictly speaking -- recommends the use of the 3DES and AES algorithms\cite{nist}, in their report for use of the 3DES algorithm they write:
				\begin{quote}
				...
				As a consequence it is decided, that the AES algorithm will be used for protecting the private-key, stored on the server. While AES supports a number of different key sizes, AES-256 is chosen for the added security.
			...
			\subsubsection{Creating an Encryption Key}
				...
				Since Argon2 is such a new algorithm, ``solid'' implementations might very well be difficult to find. But this project will adhere to OWASP's guidelines, as far as possible. As such, it is preferable if the project derives the symmetric encryption key using the Argon2 algorithm.
	...
	\section{Attaining Pseudo-Zero-Knowledge}
		...
		The authentication password is the one which is stored on the server, in its salted and hashed form \emph{(see \ref{sec:password} on page \pageref{sec:password})}. This is sent -- in clear text -- to the server for verification purposes. The decryption password us used, with an salt, to derive the symmetric encryption key, used for the client to decrypt the private-key \emph{(see \ref{sec:keys} on page \pageref{sec:keys})}. At no point in time will the front- or back-end attempt to compare these two passwords, however, it is \emph{strongly} recommended that they be different. As long as they are different it effectively ensures that the back-end can \emph{not} infer the correct decryption key, and as such can never actually inspect the passwords stored on it. This property is dubbed Pseudo-Zero-Knowledge.
	...
	\section{User Structure \& Creation}
		\label{sec:user:diff}
		regular and admin user



		\subsection{Creating a User}
			Normally, in a solution like one being designed here, creating the user would be a simple HTTP POST containing the user information. However, the requirements state that this should not be possible. Since this system will be run by individuals, the owner needs control over exactly \emph{which} users are being created.

			As such, creating a user will need an ``invite'' of sorts. 



		\subsection{Breaking the RESTful Principles On Purpose}
