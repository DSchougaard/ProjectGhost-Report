\chapter{Implementing the System: The Back-End}
	...
	\section{API Endpoints}
		...
		\subsection{The Audit Collection}
			The Audit collection is extremely simple. It only consists of a single endpoint: Reading the audit log. Since this collection is owned by individual users, this is -- much like passwords and categories -- stored behind the users collection and a specific ID. This can be seen on table \ref{tab:api:audit} on page \pageref{tab:api:audit}.

		\subsection{The Auth Collection}
			\label{sec:api:auth}
			The Auth collection is a little bit more interesting. Of course, there is the generic \verb=POST=, containing the user's username and password, for authentication purposes. The details of this is covered in section \ref{sec:design:authentication} on page \pageref{sec:design:authentication}. However, this is not all. Since the solution supports TOTP two-factor-authentication, as described in section \ref{sec:mfa} on page \pageref{sec:mfa}, endpoints for enabling this is needed as well. 
		...
		\begin{table}[p]
			...
			\caption{API endpoints for the Users collection.}
			\label{tab:api:users}
		\end{table}
		...
		\begin{table}[p]
			...
			\caption{API endpoints for the Passwords collection.}
		...
		\begin{table}[p]
			...
			\caption{API endpoints for the Categories collection.}
		...
		\begin{table}[p]
			...
			\caption{API endpoints for the Invites collection.}
		...
		\begin{table}[p]
			...
			\caption{API endpoint for the Audit collection.}
			\label{tab:api:audit}
		\end{table}
		...
		\begin{table}[p]
			...
			\caption{API endpoints for the Auth collection.}
			\label{tab:api:auth}
		\end{table}
...
%
	\section{Validating Input}
		...
		\subsection{Using ajv}
			...
			\begin{lstlisting}[gobble=16,language=JavaScript,caption={JSON schema for the password model, using ajv's notation.},label={lst:json:ajv:password},breakatwhitespace=false]
                var schema = {
                    "$async": true,
                    "properties": {
                        "title": {
	...
	\section{Middlewares}
		...
		\begin{lstlisting}[gobble=12,language=JavaScript,caption={Ping endpoint with the log middleware},label={lst:example:ping_wlog}]
	...
	\section{Authorisation}
		...
		\subsection{Basic Authorization}
			...
			In the previous too sections, the token verification \emph{(section \ref{sec:impl:token-verification})} and object resolving  \emph{(section \ref{sec:impl:resolve})} middleware were explained. Using these resolved objects' IDs, a very simple authorisation scheme can be devised.
			...
			If the above snippets are evaluated to \verb=true=, the code keeps executing. If it is evaluated to false, the back-end returns HTTP code \verb=403=, denoting that the user does \emph{not} have sufficient privileges to access that specific data.
		...
		\subsection{Simple or Advanced}
			There is not a seconds doubt that the advanced approach is more fine grained. Where the simple approach only supports differentiating between targets, the advanced approach also supports differentiating between actions. As such, while the simple method does its job perfectly as is, should the need for more detailed authorisation options arise, they will need to be implemented the advanced approach.
