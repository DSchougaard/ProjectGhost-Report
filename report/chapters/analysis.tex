...
\label{chap:analysis}
	...
	\section{Existing Available Tools}
		...
		\subsection*{In-Browser Password Managers}
			...
			In \cite{browser_saved} they review the different browsers' storage format for their respective password managers. Their primary concern is, that while the passwords might be encrypted, the decryption key is stored directly on disk. From a security perspective, this is horrible. If anyone could dump both files, complete access to all passwords would be given. They note Firefox and Opera as the only browsers supporting a Master Password, which is to be entered every time the user accesses a password stored.	However, their analysis could very well be deprecated by now. The version of Chrome, for instance, they're using is $V21.0$, while the current newest version is $V47.0$. 
	...
	\section{Academic Research and Tools}
		...
		\begin{itemize}
			...
			%\item A Convenient Method for Securely Managing Passwords
			\item Kamouflage: Loss-Resistant Password
			\item Sesame: A Secure and Convenient Mobile Solution for Passwords
		\end{itemize}
		%Covered
		%Tapas: Design, Implemetation, and Usability Evaluation of a Password Manager.pdf
		%Using CardSpace as a Password Manager.pdf
		%Implementing PassCard — a CardSpace-based Password Manager.pdf
		%Stronger Password Authentication Using Browser Extensions.pdf
		%Kamouflage: Loss-Resistant Password.pdf
		...
		%Non-Listed
		%IDKeeper: A Web Password Manager with Roaming Capability Based on USB Key .pdf
		%A Convenient Method for Securely Managing Passwords.pdf
		...
		\subsection*{Other Note Worthy Sources}
			All Your Browser-saved Passwords Could Belong to Us: a Security Analysis and a Cloud-based New Design describes how to change the current in-browser password managers, to work more safely, using a ``Secure \& Reliable Storage'' service, located in the cloud. While this
